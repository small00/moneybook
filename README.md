...
- 统计报表：月度收支汇总、分类占比环形图、近 6 个月收支柱状图、分类排行（点击查看分类明细）
- 预算管理：月度总预算 + 分类预算，超支/接近超支提醒
- 分类管理：自定义分类（emoji 图标）、分类排序（↑↓）、记账时分类折叠为两行
- 数据备份：导出 JSON / 导入恢复 / **GitHub 云备份**（一键备份到私有仓库、记账后自动备份）
- 深色模式：跟随系统自动切换
...
- v1.1.0（2026-09）：GitHub 云备份（私有仓库）、分类明细查看、SW 绕过缓存更新即时生效
